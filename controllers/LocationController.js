...
const LocationController = {
    ...
    async getByCapacity(req, res) {
        try {
            ...
            const location = await Location.findOne({ where: { capacity: capacity } });
            if (location) {
                res.send({message: "Encontrates el evento que estas buscando", location: location });
            } else {
                res.status(404).send({ message: `No encontramos ninguno evento con la capacidad para ${capacity} personas.`, location: location });
            }
        } catch (error) {
    ...
    async getInDesc(req, res) {
        try {
            ...
            res.status(200).send({message: "Eventos en orden decreciente por capacidad (personas)", location: location});
        } catch (error) {
    ...
    async getInAsc(req, res) {
        try {
            ...
            res.status(200).send({ message: "Eventos en orden creciente por capacidad (personas)", location: location});
        } catch (error) {
            console.error("Intente novamente", error);
            res.status(500).send("No encontramos nada, lo siento!");
        }
