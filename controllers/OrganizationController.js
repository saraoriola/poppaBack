...
const OrganizationController = {
    async createOrganization(req, res) {
        try {
            const { name, web, email, tel, representative, patronage } = req.body;

            if (!name || !web || !email || !tel || !representative || patronage === undefined) {
                return res.status(400).send({ message: "Debes completar todos los campos" });
            }

          const organization = await Organization.create(req.body);  // ERROR SALTA AQUÍ
