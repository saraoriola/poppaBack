...
const EventController = {
    async create(req, res) {
        try {
            const { location_id, dateTime, duration_min, type, banner, description, title } = req.body;

            if (!location_id || !dateTime || !duration_min || !type || !banner || !description || !title) {
    ...
    async getAllEvents(req, res) {
        try {
            ...
            console.error(error);
            res.status(500).send({ message: "Ha habido un problema al obtener la lista de eventos" });
        }
    },
